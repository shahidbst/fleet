<!-- DO NOT EDIT. This document is automatically generated by running `make fleetd-tuf`. -->
# tuf.fleetctl.com

Following are the currently deployed versions of fleetd components on the `stable` and `edge` channel.

## `stable`

| Component\OS |    macOS     | Linux  | Windows |
|--------------|--------------|--------|---------|
| orbit        | 1.34.0       | 1.34.0 | 1.34.0  |
| desktop      | 1.34.0       | 1.34.0 | 1.34.0  |
| osqueryd     | 5.13.1       | 5.13.1 | 5.13.1  |
| nudge        | 1.1.10.81462 | -      | -       |
| swiftDialog  | 2.1.0        | -      | -       |

## `edge`

| Component\OS | macOS  | Linux  | Windows |
|--------------|--------|--------|---------|
| orbit        | 1.35.0 | 1.35.0 | 1.35.0  |
| desktop      | 1.35.0 | 1.35.0 | 1.35.0  |
| osqueryd     | 5.14.1 | 5.14.1 | 5.14.1  |
| nudge        | -      | -      | -       |
| swiftDialog  | -      | -      | -       |
